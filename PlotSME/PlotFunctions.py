...
def PlotElement(element, DATA, plotdir):
    ...
    if element in Elements:
        ...
        ax[0].plot(OPTICAL_COMPLETE['[Fe/H]'].values,OPTICAL_COMPLETE['{el}'.format(el=element)].values,'.',color='silver',label='Complete optical')
        for i in range(3):
            ax[i].axvline(0,0,1, color='0.75', linestyle='dashed')
            ax[i].axhline(0,0,1, color='0.75', linestyle='dashed')
            ax[i].set_xlim([-2,1])
    ...
    if element == 'Alpha':
        ...
        for row in range(len(AlphaElements)):
            ax[row,0].plot(OPTICAL_COMPLETE['[Fe/H]'].values,OPTICAL_COMPLETE['{el}'.format(el=AlphaElements[row])].values,'.',color='silver',label='Complete optical')
            for i in range(3):
                ax[row,i,].axvline(0,0,1, color='0.75', linestyle='dashed')
                ax[row,i].axhline(0,0,1, color='0.75', linestyle='dashed')
                ax[row,i].set_xlim([-2,1])
